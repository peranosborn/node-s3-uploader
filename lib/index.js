...
Upload.prototype._getRandomPath = function(opts) {
  var i, input, j, path, res, x, y;
...
Upload.prototype._getDestPath = function(prefix, callback) {
  return retry(5, (function(_this) {
    return function(cb) {
      var path;
      path = prefix + _this._getRandomPath(_this.opts);
      return _this.s3.listObjects({
        ...
        if (data.Contents.length === 0 || !_this.opts.useRandomPath) {
          return cb(null, path);
        }
...
Image.prototype.uploadVersions = function(cb, results) {
  if (this.upload.opts.original) {
    results.versions.push({
      awsImageAcl: this.upload.opts.original.awsImageAcl,
      original: true,
...
Image.prototype._upload = function(dest, version, cb) {
  var filename, format, options;
  filename = this.upload.opts.fileName || '';
  ...
  options = {
    Key: "" + dest + (version.versionPrefix || '') + filename + (version.suffix || '') + "." + format,
